...
void ClockWidget::Draw() {
    ...
    DrawTextBCL(tx.c_str(),x+5, y, height/2, height, textColor);

    // Draw pause icon

    if(paused){
        float triangleSize = height/4;
        Vector2 triangleMidPoint = {static_cast<float>(x + width - triangleSize - triangleSize/2),
                                    static_cast<float>(y + height - height/2)};
        DrawTriangle({triangleMidPoint.x - triangleSize/4, y + triangleSize},
                     {triangleMidPoint.x - triangleSize/4, y + height - triangleSize},
                     {triangleMidPoint.x + triangleSize, triangleMidPoint.y},
                     isHovering ? color : hoverColor);
    }else{
        DrawRectangle(x + width - height/4 - height/4, y+height/4, height/8, height - height/2, isHovering ? color : hoverColor);
        DrawRectangle(x + width - height/4, y+height/4, height/8, height - height/2, isHovering ? color : hoverColor);
    }
